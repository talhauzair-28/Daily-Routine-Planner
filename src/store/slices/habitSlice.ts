...
const habitSlice = createSlice({
  ...
  reducers: {
    ...
    updatePrayer: (
      state,
      action: PayloadAction<{ prayerId: string; updates: Partial<Prayer> }>
    ) => {
    ...
    updateZikrSession: (
      ...
      action: PayloadAction<{
        sessionId: string;
        updates: Partial<ZikrSession>;
      }>
    ) => {
      if (state.todayRecord) {
        const session = state.todayRecord.zikrSessions.find(
    ...
    updateQuranSession: (
      state,
      action: PayloadAction<Partial<QuranSession>>
    ) => {
      if (state.todayRecord) {
    ...
    updateFamilyTime: (
      state,
      action: PayloadAction<{ id: string; updates: Partial<FamilyTime> }>
    ) => {
      if (state.todayRecord) {
        const familyTime = state.todayRecord.familyTime.find(
    ...
    updateSleep: (
      state,
      action: PayloadAction<Partial<DailyRecord['sleep']>>
    ) => {
      if (state.todayRecord) {
    ...
    saveDailyRecord: state => {
      if (state.todayRecord) {
        const existingIndex = state.monthlyRecords.findIndex(
          r => r.date === state.todayRecord!.date
        );
        if (existingIndex >= 0) {
