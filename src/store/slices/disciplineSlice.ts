...
const disciplineSlice = createSlice({
  ...
  reducers: {
    initializeTodayChallenges: (
      state,
      action: PayloadAction<{ workType: 'office' | 'wfh' | 'weekend' }>
    ) => {
      ...
      baseChallenges.push(
        {
          id: 'evening-zikr-start',
          name: 'Start Evening Zikr',
          category: 'timing',
          targetTime: eveningZikrTime,
          onTime: false,
          streak: 0,
          success: false,
        },
    ...
    updateChallenge: (
      ...
      action: PayloadAction<{
        challengeId: string;
        updates: Partial<DisciplineChallenge>;
      }>
    ) => {
      const challenge = state.todayChallenges.find(
    ...
    calculateDailyScores: state => {
      const timingChallenges = state.todayChallenges.filter(
        c => c.category === 'timing'
      );
      const spiritualChallenges = state.todayChallenges.filter(
        c => c.category === 'spiritual'
      );
      const familyChallenges = state.todayChallenges.filter(
        c => c.category === 'family'
      );
      const personalChallenges = state.todayChallenges.filter(
        c => c.category === 'personal'
      );

      state.dailyScores.timing = timingChallenges.filter(c => c.success).length;
      state.dailyScores.spiritual = spiritualChallenges.filter(
        c => c.success
      ).length;
      state.dailyScores.family = familyChallenges.filter(c => c.success).length;
      state.dailyScores.personal = personalChallenges.filter(
        c => c.success
      ).length;
      state.dailyScores.total = state.todayChallenges.filter(
        c => c.success
      ).length;
    },
    ...
    updateStreaks: (
      state,
      action: PayloadAction<{ [challengeId: string]: number }>
    ) => {
